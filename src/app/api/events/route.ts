...
export async function GET() {
  try {
    ...
    const events = jobs.map((job: any) => ({
      id: job.id,
      title: job.name,
      date: job.schedule?.at ? new Date(job.schedule.at).toISOString().split("T")[0] : null,
      time: job.schedule?.at ? new Date(job.schedule.at).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }) : null,
      type: job.name.toLowerCase().includes("payroll") ? "payroll" : 
            job.name.toLowerCase().includes("reminder") ? "reminder" : "scheduled",
